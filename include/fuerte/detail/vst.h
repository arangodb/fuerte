...
struct ChunkHeader {
  ...
  inline uint64_t messageLeng  std::string const length("\x98\0\0\0", 4); // 152 = 24 + 128 bytes length
  std::string const chunkX_0("\x03\0\0\0", 4); // 3 chunks = ((0b01 << 1) | 1)
  std::string const mid("\x01\0\0\0\0\0\0\x01", 8); // messageId
  std::string const mLength("\x80\0\0\0\0\0\0\0", 8); // messageLength
  std::string const data(128, 'a'); // messageId 1

  std::string chunk0 = length + chunkX_0 + mid + mLength + data;

  ASSERT_EQ(chunk0.size(), 152);
  ...
  ASSERT_EQ(chunkLength, 152);
  auto resultChunk = fu::vst::parser::readChunkHeaderVST1_1(ptr);
  ASSERT_EQ(resultChunk.heade  std::string const length("\x98\0\0\0", 4); // 152 = 24 + 128 bytes length
  std::string const chunkX_0("\x03\0\0\0", 4); // 3 chunks = ((0b01 << 1) | 1)
  std::string const mid("\x01\0\0\0\0\0\0\x01", 8); // messageId
  std::string const mLength("\x80\0\0\0\0\0\0\0", 8); // messageLength
  std::string const data(128, 'a'); // messageId 1
...
r.chunkLength(), 152);
  ...
  std::string const length("\x98\0\0\0", 4); // 152 = 24 + 128 bytes length
  std::string const chunkX_0("\x03\0\0\0", 4); // 3 chunks = ((0b01 << 1) | 1)
  std::string const mid("\x01\0\0\0\0\0\0\x01", 8); // messageId
  std::string const mLength("\x80\0\0\0\0\0\0\0", 8); // messageLength
  std::string const data(128, 'a'); // messageId 1
...
th() const { return _messageLength; }
