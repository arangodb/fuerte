...
void curlpp::terminate() { curl_global_cleanup(); }

curlpp::Cleanup::Cleanup() { curlpp::initialize(); }

curlpp::Cleanup::~Cleanup() {}

std::string curlpp::escape(const std::string& url) {
  std::string buffer;
...
std::string curlpp::unescape(const std::string& url) {
  std::string buffer;
...
std::string curlpp::getenv(const std::string& name) {
  std::string buffer;
  char* p = curl_getenv(name.c_str());
  if (!p) {
    throw RuntimeError(
        "unable to get the environnement string");  // we got an error
  } else {
    buffer = p;
    free(p);
  }
...
std::string curlpp::libcurlVersion() {
  char* p = curl_version();
  if (!p) {
...
time_t curlpp::getdate(const std::string& date, time_t* now) {
