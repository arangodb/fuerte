...
VstConnection::VstConnection(std::shared_ptr<boost::asio::io_context> const& ctx, 
...
std::unique_ptr<RequestItem> VstConnection::createRequestItem(std::unique_ptr<Request> request, 
                                                              RequestCallback cb) {
...
void VstConnection::finishInitialization() {
  ...
  boost::asio::async_write(*_socket, boost::asio::buffer(vstHeader, strlen(vstHeader)),
...
bool VstConnection::asyncReadCallback(const boost::system::error_code& e, std::size_t transferred) {
  ...
  if (e) {
    ...
    auto cursor = boost::asio::buffer_cast<const uint8_t*>(recvBuffs); // TODO this is deprecated
