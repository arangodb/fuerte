...
HttpConnectionOld::HttpConnectionOld(EventLoopService& eventLoopService, ConnectionConfiguration const& configuration)
    : Connection(eventLoopService, configuration) {
  _curlm.reset(new CurlMultiAsio(
      *eventLoopService.io_context(), 
        boost::bind(&HttpConnection::handleResult, this, _1, _2)));
}

HttpConnectionOld::~HttpConnectionOld() { 
  FUERTE_LOG_HTTPTRACE << "Destroying HttpConnectionOld" << std::endl;
...
MessageID HttpConnectionOld::sendRequest(std::unique_ptr<Request> request, RequestCallback callback) {
...
uint64_t HttpConnectionOld::queueRequest(Destination destination,
...
int HttpConnectionOld::curlDebug(CURL* handle, curl_infotype type, char* data,
...
void HttpConnectionOld::logHttpBody(std::string const& prefix,
...
void HttpConnectionOld::logHttpHeaders(std::string const& prefix,
...
size_t HttpConnectionOld::readHeaders(char* buffer, size_t size, size_t nitems,
                                     void* userptr) {
...
size_t HttpConnectionOld::readBody(void* data, size_t size, size_t nitems,
                                  void* userp) {
  size_t realsize = size * nitems;
...
void HttpConnectionOld::transformResult(CURL* handle, StringMap&& responseHeaders,
...
std::string HttpConnectionOld::createSafeDottedCurlUrl(
...
void HttpConnectionOld::createRequestItem(const Destination& destination, std::unique_ptr<Request> request, RequestCallback callback) {
  ...
  curl_easy_setopt(handle, CURLOPT_WRITEFUNCTION, HttpConnectionOld::readBody);
  curl_easy_setopt(handle, CURLOPT_WRITEDATA, requestItem.get());
  curl_easy_setopt(handle, CURLOPT_HEADERFUNCTION, &HttpConnectionOld::readHeaders);
  curl_easy_setopt(handle, CURLOPT_HEADERDATA, requestItem.get());
#if ENABLE_FUERTE_LOG_HTTPRACE > 0
  curl_easy_setopt(handle, CURLOPT_DEBUGFUNCTION, HttpConnectionOld::curlDebug);
...
void HttpConnectionOld::handleResult(CURL* handle, CURLcode rc) {
...
#endif
        requestItem->Er(0, std::move(requestItem->_request), std::move(fuResponse));
