...
SList::SList(const SList& rhs) : mList(0), mData(rhs.mData) { update(); }

SList::SList(curl_slist* list) : mList(NULL) { constructFrom(list); }

SList::SList(const std::list<std::string>& rhs) : mList(0), mData(rhs) {
  update();
}
...
SList::operator std::list<std::string>() { return list(); }
...
curl_slist* SList::cslist() const { return mList; }

std::list<std::string> SList::list() { return mData; }
