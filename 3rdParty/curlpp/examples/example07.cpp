...
class YesNo {
 public:
  ...
  std::string operator()() const { return yesno ? "Yes" : "No"; }
...
std::ostream& operator<<(std::ostream& strm, const MyCookie& cook) {
  strm << "Cookie: '" << cook.name << "' (secure: " << YesNo(cook.secure)
       << ", tail: " << YesNo(cook.tail) << ") for domain: '" << cook.domain
       << "', "
       << "path: '" << cook.path << "'.\n";
...
std::vector<std::string>& split_cookie_str(const std::string& str,
...
int StrToInt(const std::string& str) {
  ...
  int i = 0;
  if (!(strm >> i)) {
    throw curlpp::RuntimeError("Unable to convert string '" + str +
                               "' to integer!");
  }
  return i;
}
...
int main(void) {
  try {
    ...
    cookieList.push_back(
        "Set-Cookie: GMAIL_AT=EXPIRED;expires=Sun, 17-Jan-2038 19:14:07 GMT; "
        "path=/; domain=.google.com");

    // a Netscape style cookie with \t
    cookieList.push_back(
        ".google.com\tTRUE\t/\tFALSE\t2147483647\tLSID\tI like you GOOGLE");

    // a Netscape style cookie with tabs in string
    cookieList.push_back(
        ".yahoo.com	TRUE	/	FALSE	0	YAHOO_COOKIE	I like "
        "you yahoo, too");
